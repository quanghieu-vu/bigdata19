...
class FeatureManagement():
    ...
    def __init__(self):
        ...
        self.oe_columns = [
            ...
            "lookup_srcport_dominate",
            "lookup_dstport_dominate"
        ]
    ...
    def process(self):
        ...
        self.df_all["ip_zone_34"] = self.df_all.apply(lambda row: utils.concatenate_values([row["ip_zone_3"], row["ip_zone_4"]]), axis=1)
        self.df_all["ip_zone_234"] = self.df_all.apply(lambda row: utils.concatenate_values([row["ip_zone_2"], row["ip_zone_3"], row["ip_zone_4"]]), axis=1)
        ...
        for item in feature_pairs:
            ...
            f2 = item[1]
            fn = f1 + "_" + f2
            self.df_all[fn] = self.df_all.apply(lambda row: utils.concatenate_values([row[f1], row[f2]]), axis=1)
