...
from feature_management_lgb import FeatureManagement
...
class DataPreprocessing():
    ...
    def __init__(self):
        ...
        self.lookup_protocols = sorted(["tcp", "udp", "ssh", "https", "ftp", "dns", "imap", "pop", "ntp", "smtp"])
        self.lookup_devicetypes = sorted(["fw", "dns", "nids", "faz", "nips"])
        self.lookup_dstports = ["l100", "l200", "l500"]
        self.lookup_srcports = ["l100", "l200", "l500"]

    def eda(self):
        for column in ["alerttype", "devicetype", \
            "reportingdevice_code", "devicevendor_code", \
    ...
    def get_values(self, column):
        if column == "lookup_protocol":
            ...
            '''
            limit = len(self.lookup_protocols)
    ...
    def build_alert_dict(self):
        ...
        dominate_columns = ["severity", "devicetype", "devicevendor_code", "protocol", \
                            "alerttype", "srcport", "dstport", "lookup_protocol", \
                            "lookup_dstport", "lookup_srcport"]
        ...
        while True:
            ...
            matched_ipcategory = 0
            matched_portcategory = 0
            if params[dict_mappings["srcipcategory"]] == params[dict_mappings["dstipcategory"]]:
                matched_ipcategory = 1
            if params[dict_mappings["srcportcategory"]] == params[dict_mappings["dstportcategory"]]:
                matched_portcategory = 1

            lookup_protocol = "others"
            for protocol in self.lookup_protocols:
                if protocol in params[dict_mappings["protocol"]]:
            ...
            if alert_id not in alert_dict:
                ...
                alert_dict[alert_id]["alerttime_count"] = 0
                alert_dict[alert_id]["count"] = 0
                alert_dict[alert_id]["severity_level"] = 0
                alert_dict[alert_id]["matched_ipcategory"] = 0
                alert_dict[alert_id]["matched_portcategory"] = 0
            ...
            alert_dict[alert_id]["alerttime_count"] += 1
            alert_dict[alert_id]["count"] += count
            alert_dict[alert_id]["severity_level"] += severity_level
            alert_dict[alert_id]["matched_ipcategory"] += matched_ipcategory
            alert_dict[alert_id]["matched_portcategory"] += matched_portcategory
        ...
        fout.write("alert_ids,alerttime_sum,alerttime_count,count_sum")
        fout.write(",matched_ipcategory,matched_portcategory")

        for key in dict_sorted_keys:
            fout.write(",{}_cnt".format(key))
        fout.write(",srcipscope_cnt,dstipscope_cnt,lookup_protocol_cnt")
        ...
        fout.write(",domain_0,domain_1,username_0,username_1,signature_0,signature_1")
        ...
        for alert_id in sorted(alert_dict.keys()):
            ...
            alerttime_count = alert_dict[alert_id]["alerttime_count"]
            count_sum = alert_dict[alert_id]["count"]
            fout.write("{},{},{},{}".format(alert_id, alerttime_sum, alerttime_count, count_sum))

            matched_ipcategory = alert_dict[alert_id]["matched_ipcategory"]
            matched_portcategory = alert_dict[alert_id]["matched_portcategory"]
            fout.write(",{},{}".format(matched_ipcategory, matched_portcategory))

            for key in dict_sorted_keys:
                fout.write(",{}".format(len(alert_dict[alert_id][key])))
            ...
            for column in ["domain", "username", "signature"]:
                if "0" in alert_dict[alert_id][column]:
                    ...
                    fout.write(",0")
                if "1" in alert_dict[alert_id][column]:
                    fout.write(",{}".format(alert_dict[alert_id][column]["1"]))
                else:
                    fout.write(",1")
